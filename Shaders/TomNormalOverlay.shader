Shader "xukmi/TomNormalOverlay"
{
	Properties
	{
		...
		_SlopeThreshold ("Normal Slope Threshold", Range(0, 1)) = 0.25
		_SlopeContrast ("Normal Slope Contrast", Range(0.1, 8)) = 3
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f i, int faceDir : VFACE) : SV_Target
			{
				...
				float slopeHighlight = ThresholdContrast(pow(normalSlope, 0.75), _SlopeThreshold, _SlopeContrast);
				float facing = pow(saturate(dot(normalize(i.normalWS), v)), _FacingBoostPower);

				float atten = LIGHT_ATTENUATION(i);
				float3 ggx = EvaluateGGX(i, n, v, l, _LightColor0.rgb, atten) * _GGXIntensity * _DirectIntensity;
				...
				for (int lightIndex = 0; lightIndex < 4; lightIndex++)
				{
					...
					ggx += EvaluateGGX(i, n, v, vertexLight.dir, vertexLight.col.rgb, vertexLight.atten) * _GGXIntensity * _PointLightIntensity;
				}
			#endif

				float wrappedNdotH = saturate(dot(n, h) * 0.5 + 0.5);
				float fakeSpec = pow(lerp(saturate(dot(n, h)), wrappedNdotH, _FakeSpecWrap), _FakeSpecPower);
				float3 fakeCol = fakeSpec * _FakeSpecColor.rgb * _FakeSpecIntensity;

				float3 matcap = SampleOverlayMatCap(n);
				matcap = pow(max(matcap, 1E-06), 0.454545) * _MatCapIntensity;
				...
				float3 slopeCol = slopeHighlight * _FakeSpecColor.rgb * _SlopeIntensity;
				float3 facingCol = slopeHighlight * facing * _FakeSpecColor.rgb * _FacingBoostIntensity;
				float3 finalCol = (ggx + fakeCol + slopeCol + facingCol + matcap * (_MatCapFrontStrength + fresnel)) * _OverlayTint.rgb;

				float alphaSource = Luminance(ggx) * _GGXAlpha + fakeSpec * _FakeSpecAlpha + slopeHighlight * _SlopeAlpha + slopeHighlight * facing * _FacingBoostAlpha + matcapAlpha;
				float alpha = saturate(alphaSource * _OverlayAlpha * mask * _OverlayTint.a);
