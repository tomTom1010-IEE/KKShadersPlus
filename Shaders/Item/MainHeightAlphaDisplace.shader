Shader "xukmi/MainHeightAlphaDisplace"
{
	Properties
	{
		...
		_HeightScale ("Height Normal Scale", Float) = 8
		_HeightThreshold ("Height Cutoff Threshold", Range(0, 1)) = 0.01
		_HeightContrast ("Height Cutoff Contrast", Float) = 32
		...
		_NormalMapScale ("Height Normal Map Relative Scale", Float) = 1
		...
		[Enum(UnityEngine.Rendering.BlendMode)] _BlendSrc ("Main Blend Src", Float) = 5
		[Enum(UnityEngine.Rendering.BlendMode)] _BlendDst ("Main Blend Dst", Float) = 10
		...
		[MaterialToggle] _UseNormalMapForMatCap ("Use Height Normal For MatCap", Float) = 1
		...
		_HeightDisplaceStrength("Height Displace Strength", float) = 0
		_HeightDisplaceNormalStrength("Height Displace Normal Strength", float) = 1
	...
	SubShader
	{
		...
		Pass
		{
			...
			#include "KKPItemHeightDisplace.cginc"
			...
			Varyings vert (VertexData v)
			{
				...
				HeightDisplacementValues(v, vertex, normal);
				v.vertex = vertex;
				v.normal = normal;
			...
			fixed4 outlineFrag (Varyings i) : SV_Target
			{
				...
				AlphaClip(i.uv0, _OutlineOn ? mainTex.a * _Alpha * GetHeightMask(i.uv0) : 0);
		...
		Pass
		{
			...
			#include "KKPItemHeightDisplace.cginc"
			#include "KKPItemDiffuse.cginc"
			#include "KKPItemHeightNormals.cginc"
			...
			#include "KKPItemHeightFrag.cginc"
		...
		Pass
		{
            ...
            float4 shadowFrag(v2f i) : SV_Target
            {
				...
				float mainTexAlpha = SAMPLE_TEX2D(_MainTex, i.uv0 * _MainTex_ST.xy + _MainTex_ST.zw).a * GetHeightMask(i.uv0);
