#ifndef KKP_ITEM_BLEND_FRAG_INC
#define KKP_ITEM_BLEND_FRAG_INC
...
DECLARE_TEX2D_NOSAMPLER(_PBRMetallicMap);
DECLARE_TEX2D_NOSAMPLER(_PBRRoughnessMap);
DECLARE_TEX2D_NOSAMPLER(_PBROcclusionMap);
DECLARE_TEX2D_NOSAMPLER(_PBRDiffuseBlendMask);
DECLARE_TEX2D_NOSAMPLER(_PBRSpecularBlendMask);
DECLARE_TEX2D_NOSAMPLER(_PBRReflectionBlendMask);
...
void EvaluatePBRMix(
	...
	out float3 pbrDiffuse,
	out float3 pbrSpecular,
	out float3 pbrReflection,
	out float diffuseBlend,
	out float specularBlend,
	out float reflectionBlend)
{
	float metallic = saturate(_PBRMetallic * SAMPLE_TEX2D_SAMPLER(_PBRMetallicMap, _MainTex, uv * _PBRMetallicMap_ST.xy + _PBRMetallicMap_ST.zw).r);
	float roughness = saturate(_PBRRoughness * SAMPLE_TEX2D_SAMPLER(_PBRRoughnessMap, _MainTex, uv * _PBRRoughnessMap_ST.xy + _PBRRoughnessMap_ST.zw).r + _PBRRoughnessBias);
	roughness = max(roughness, 0.04);

	float occlusionRaw = saturate(_PBROcclusion * SAMPLE_TEX2D_SAMPLER(_PBROcclusionMap, _MainTex, uv * _PBROcclusionMap_ST.xy + _PBROcclusionMap_ST.zw).r);
	float occlusion = lerp(1.0, occlusionRaw, _PBROcclusionStrength);

	diffuseBlend = saturate(_PBRMasterBlend * _PBRDiffuseBlend * SAMPLE_TEX2D_SAMPLER(_PBRDiffuseBlendMask, _MainTex, uv * _PBRDiffuseBlendMask_ST.xy + _PBRDiffuseBlendMask_ST.zw).r);
	specularBlend = saturate(_PBRMasterBlend * _PBRSpecularBlend * SAMPLE_TEX2D_SAMPLER(_PBRSpecularBlendMask, _MainTex, uv * _PBRSpecularBlendMask_ST.xy + _PBRSpecularBlendMask_ST.zw).r);
	reflectionBlend = saturate(_PBRMasterBlend * _PBRReflectionBlend * SAMPLE_TEX2D_SAMPLER(_PBRReflectionBlendMask, _MainTex, uv * _PBRReflectionBlendMask_ST.xy + _PBRReflectionBlendMask_ST.zw).r);

	float3 n = normalize(normal);
	float3 v = normalize(viewDir);
	float3 l = normalize(_WorldSpaceLightPos0.xyz - posWS * _WorldSpaceLightPos0.w);
	...
	float3 directLight = _LightColor0.rgb * shadowAttenuation * nDotL * _PBRDirectIntensity;
	pbrDiffuse = kd * albedo / KKP_PBR_PI * directLight;
	pbrSpecular = KKP_PBR_DirectSpecularGGX(nDotV, nDotL, nDotH, vDotH, roughness, f0, _PBRFresnelStrength) * directLight;
	...
	for (int lightIndex = 0; lightIndex < 4; lightIndex++)
	{
		...
		float3 pointKd = (1.0 - pointF) * (1.0 - metallic);

		pbrDiffuse += pointKd * albedo / KKP_PBR_PI * pointLight;
		pbrSpecular += KKP_PBR_DirectSpecularGGX(nDotV, pointNDotL, pointNDotH, pointVDotH, roughness, f0, _PBRFresnelStrength) * pointLight;
	}
#endif

	pbrDiffuse += ShadeSH9(float4(n, 1.0)).rgb * albedo * kd * occlusion;
	pbrReflection = KKP_PBR_SampleReflectionProbe(r, roughness) * KKP_PBR_FresnelSchlick(nDotV, f0, _PBRFresnelStrength) * occlusion * _PBREnvIntensity;
}
...
fixed4 frag (Varyings i, int faceDir : VFACE) : SV_Target{
	...
	float3 pbrAlbedo = max(diffuse * _PBRBaseColor.rgb, 1E-06);
	...
	float3 pbrDiffuse = 0.0;
	float3 pbrSpecular = 0.0;
	float3 pbrReflection = 0.0;
	float pbrDiffuseBlend = 0.0;
	float pbrSpecularBlend = 0.0;
	float pbrReflectionBlend = 0.0;
	EvaluatePBRMix(i.uv0, pbrAlbedo, normal, viewDir, i.posWS.xyz, shadowAttenuation, vertexLights, pbrDiffuse, pbrSpecular, pbrReflection, pbrDiffuseBlend, pbrSpecularBlend, pbrReflectionBlend);
	...
	finalDiffuse = lerp(finalDiffuse, pbrDiffuse, pbrDiffuseBlend);
	finalDiffuse = GetBlendReflections(i, max(finalDiffuse, 1E-06), normal, viewDir, kkMetalMap, finalRamp);
	finalDiffuse += pbrSpecular * pbrSpecularBlend + pbrReflection * pbrReflectionBlend;
